...
namespace WakaTime
{
    ...
    public class Plugin
    {
        ...
        static void SendHeartbeat(bool fromSave = false)
        {
            ...
            handler.PostRequest(URL_PREFIX + "users/current/heartbeats?api_key=" + _apiKey, heartbeatJSON);
            handler.OnResponse += Handler_OnResponse;
        }

        private static void Handler_OnResponse(object sender, HeartbeatResponse response)
        {
            if (sender is PostRequestHandler handler)
            {
                _lastHeartbeat = response;
                handler.Dispose();
            }
        }

        class PostRequestHandler : IDisposable
        {
            public event EventHandler<HeartbeatResponse> OnResponse;
            UnityWebRequest webRequest;
            UploadHandlerRaw uploadHandler;

            public void PostRequest(string uri, string txData)
            {
                webRequest = UnityWebRequest.Post(uri, String.Empty);
                uploadHandler = new UploadHandlerRaw(System.Text.Encoding.UTF8.GetBytes(txData));
                webRequest.uploadHandler = uploadHandler;
                webRequest.SetRequestHeader("Content-Type", "application/json");
                webRequest.SendWebRequest().completed += PostRequestHandler_completed;
            }

            private void PostRequestHandler_completed(AsyncOperation handler)
            {
                if (webRequest.result == UnityWebRequest.Result.Success)
                {
                    ...
                    var response = JsonUtility.FromJson<Response<HeartbeatResponse>>(webRequest.downloadHandler.text);
                    HandlePostResponse(response);
                }
                else
                {
                    Debug.LogError("Error while sending heartbeat code: " + webRequest.error);
                }
            }

            void HandlePostResponse(Response<HeartbeatResponse> response)
            {
                ...
                else
                {
                    ...
                    OnResponse?.Invoke(this, response.data);
                }
            }
